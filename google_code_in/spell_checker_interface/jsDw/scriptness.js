...
function spell_checker() {
	...
	for (var x = 0; x < words.length; x++){
		...
		for (var xx = 0; xx < dummy_words.length; xx++){
			if (words[x] == dummy_words[xx] || words[x] == dummy_words[xx]+"," || words[x] == dummy_words[xx]+"."){
				spelled_correct = true;
			}
		}
		if (!spelled_correct) {
			corrected_words.push(words[x]);
	...
	$('.words_to_cor').remove();
	$('.img_container').after('<p class="words_to_cor">'+corrected_words+'</p>');
	$('#textAreaId').focusin(function() {
